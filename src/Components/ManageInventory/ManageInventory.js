...
const ManageInventory = () => {
  ...
  useEffect(() => {
    (async () => {
      await axios.get(`https://cologne-perfume-server.vercel.app/inventory?pageNum=${page}&size=${size}`).then((res) => {
  ...
  const handleDeleteProduct = async (id) => {
    ...
    if (!confirm) {
      ...
      //used axios for
      await axios.delete(`https://cologne-perfume-server.vercel.app/deleteItem/${id}`).then((res) => {
