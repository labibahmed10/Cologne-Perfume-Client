...
const AddNewInventory = () => {
  ...
  const handleAddNewProduct = async (e) => {
    ...
    await axios
      .post("https://cologne-perfume-server.vercel.app/inventory", newProduct, {
        headers: {
    ...
    <div className="mt-28 md:mx-auto mx-5 md:w-[30rem] md:px-8 px-3 py-5 border rounded-lg border-[#ad6449]">
      ...
      <div className="md:mt-12 mt-8">
        <form onSubmit={handleAddNewProduct}>
          ...
          <input className="w-full py-3 mb-3 px-5 focus:outline-none bg-slate-50 rounded-lg" type="number" name="quantity" placeholder="Quantity" />
          <input className="w-full py-3 mb-3 px-5 focus:outline-none bg-slate-50 rounded-lg" type="text" name="supplier" placeholder="Supplier" />
          <input
